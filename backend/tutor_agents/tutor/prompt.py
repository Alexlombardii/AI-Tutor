...
#   {
#     id: "RAG-001",
#     name: "A-Level Maths - product rule",
#     topic: "product rule differentiation formula and worked examples",
#     content: "The product rule states that if you have two functions u(x) and v(x), then d/dx[u(x)v(x)] = u'(x)v(x) + u(x)v'(x). For example, to differentiate f(x) = x²sin(x): Let u = x² and v = sin(x). Then u' = 2x and v' = cos(x). Using the product rule: f'(x) = (2x)(sin(x)) + (x²)(cos(x)) = 2x sin(x) + x² cos(x).",
#     score: 0.95,
#     source: "A-Level Mathematics Textbook"
#   },
#   {
#     id: "RAG-002", 
#     name: "A-Level Maths - product rule",
#     topic: "product rule differentiation formula and worked examples",
#     content: "Common mistakes with the product rule include forgetting to differentiate both functions or mixing up the order. Remember: derivative of first × second + first × derivative of second. Practice with functions like (3x + 1)(x² - 2) where u = 3x + 1, u' = 3, v = x² - 2, v' = 2x.",
#     score: 0.87,
#     source: "A-Level Mathematics Textbook"
#   }
# ];
...
# # Message
# The product rule helps you differentiate when you have two functions multiplied together. The formula is: if f(x) = u(x) × v(x), then f'(x) = u'(x)v(x) + u(x)v'(x) [A-Level Maths - product rule](RAG-001). For example, with x²sin(x), you get 2x sin(x) + x² cos(x). Would you like me to work through another example step by step?
# """
